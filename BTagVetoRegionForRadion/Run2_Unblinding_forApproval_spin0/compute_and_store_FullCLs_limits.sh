...
for MASS in "$@"; do
    ...
    FINAL_MERGED_FILE="${WORKDIR}/BatchMerged_output/FinalMergedFull/final_merged.root"
    COMBINE_INPUT="../../combined_run2_${MASS}.root"
    OUTFILE="${WORKDIR}/limits_mass_${MASS}.json"   # keep in mass folder
    ...
    MERGED_DIR="$(dirname "$FINAL_MERGED_FILE")"
    pushd "$MERGED_DIR" > /dev/null || exit 1
